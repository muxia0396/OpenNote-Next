...
fun LiteTextField(
    ...
    fun applyChange(tfv: TextFieldValue) {
        textFieldValue = tfv
        if (state.text.toString() != tfv.text) {
            state.edit {
                replace(0, length, tfv.text)
    ...
    LaunchedEffect(state.text) {
        val externalText = state.text.toString()
        if (externalText != textFieldValue.text) {
            val selection = textFieldValue.selection.run {
                TextRange(
                    start.coerceIn(0, externalText.length),
                    end.coerceIn(0, externalText.length)
                )
            }
            textFieldValue = TextFieldValue(text = externalText, selection = selection)
        }
    }
