...
import com.yangdai.opennote.presentation.component.note.ScrollDirection
...
fun NoteScreen(
    ...
    val syncedSearchQuery by viewModel.noteSearchQueryFlow.collectAsStateWithLifecycle()
    ...
    val contentLoadState by viewModel.contentLoadStateFlow.collectAsStateWithLifecycle()
    val appSettings by viewModel.settingsStateFlow.collectAsStateWithLifecycle()
    val scrollState = rememberScrollState()

    fun currentReadProgress(): Float = if (scrollState.maxValue > 0) {
    ...
    DisposableEffect(Unit) {
        ...
        onDispose {
            val progress = currentReadProgress()
            if (!viewModel.shouldShowSnackbar()) {
                viewModel.onNoteEvent(NoteEvent.SaveOrUpdate(progress))
            } else {
                viewModel.onNoteEvent(NoteEvent.UpdateReadProgress(progress))
            }
        }
    }

    var hasRestoredReadPosition by remember(noteId) { mutableStateOf(false) }
    LaunchedEffect(
        noteState.id,
        noteState.lastReadProgress,
        contentLoadState.loading,
        scrollState.maxValue
    ) {
        if (!hasRestoredReadPosition &&
            noteState.id == noteId &&
            !contentLoadState.loading
        ) {
            if (scrollState.maxValue > 0 && noteState.lastReadProgress > 0f) {
                scrollState.scrollTo(
                    (scrollState.maxValue * noteState.lastReadProgress)
                        .roundToInt()
                        .coerceIn(0, scrollState.maxValue)
                )
            }
            if (scrollState.maxValue > 0 || noteState.lastReadProgress == 0f) {
                hasRestoredReadPosition = true
            }
        }
    }
    ...
    LaunchedEffect(syncedSearchQuery, noteState.id) {
        if (syncedSearchQuery.isNotBlank() && noteState.id == noteId) {
            isSearching = true
            searchState = FindAndReplaceState(
                searchWord = syncedSearchQuery,
                scrollDirection = ScrollDirection.NEXT
            )
        }
    }
    LaunchedEffect(searchState.searchWord, viewModel.contentState.text) {
        if (searchState.searchWord.isBlank()) {
            if (searchState.matchCount != 0) searchState = searchState.copy(matchCount = 0)
            return@LaunchedEffect
        }
        withContext(Dispatchers.Default) {
            searchState = searchState.copy(
                matchCount = Regex(
                    Regex.escape(searchState.searchWord),
    ...
    var creationTimestamp by rememberSaveable { mutableStateOf("") }
    ...
    LaunchedEffect(noteState) {
        withContext(Dispatchers.Default) {
            ...
            creationTimestamp = dateTimeFormatter.format(
                noteState.createdAt ?: noteState.timestamp ?: System.currentTimeMillis()
            )
            timestamp =
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ...
                    if (!noteState.openAsExternalHtml) {
                        IconButtonWithTooltip(
        ...
        bottomBar = {
            if (noteState.isStandard && !noteState.openAsExternalHtml)
                AnimatedVisibility(
                    visible = !isReadView,
                    ...
                    action = {
                        IconButton(onClick = {
                            viewModel.onNoteEvent(
                                NoteEvent.SaveOrUpdate(currentReadProgress())
                            )
                            navigateUp()
                        }) {
                            Icon(
        ...
        Column(
            ...
            if (noteState.openAsExternalHtml) {
                ReadView(
                    ...
                    externalHtmlUri = noteState.sourceUri?.toUri(),
                    findAndReplaceState = searchState,
                    onFindAndReplaceUpdate = { searchState = it }
                )
            } else {
                val useLiteEditor = !noteState.isStandard &&
                        !viewModel.shouldUseLargeTextEditor()
                if (useLiteEditor) LiteTextField(
                modifier = Modifier.fillMaxSize(),
                readMode = isReadView,
    ...
    NoteSideSheet(
        ...
        drawerContent = {
            ...
            NoteSideSheetItem(
                key = stringResource(R.string.creation_time),
    ...
    ProgressDialog(
        isLoading = contentLoadState.loading || dataAction.loading,
        progress = if (contentLoadState.loading) contentLoadState.progress else dataAction.progress,
        message = dataAction.message,
        cancelable = !contentLoadState.loading,
