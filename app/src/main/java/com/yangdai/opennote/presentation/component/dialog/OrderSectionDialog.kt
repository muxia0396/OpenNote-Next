...
fun OrderSectionDialog(
    ...
    val typeOptions = listOf(
        stringResource(R.string.creation_time),
        stringResource(R.string.last_edited_time),
        stringResource(R.string.file_initial)
    )
                ...
                SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth()) {
                    SegmentedButton(
                        ...
                        onClick = {
                            ...
                            newOrder = NoteOrder.Created(newOrder.orderType)
                        },
                        selected = newOrder is NoteOrder.Created
                    ) {
                        Text(typeOptions[0])
                    }
                    SegmentedButton(
                        ...
                        onClick = {
                            hapticFeedback.performHapticFeedback(HapticFeedbackType.SegmentTick)
                            newOrder = NoteOrder.Modified(newOrder.orderType)
                        },
                        selected = newOrder is NoteOrder.Modified
                    ) {
                        Text(typeOptions[1])
                    }
                    SegmentedButton(
                        ...
                        onClick = {
                            hapticFeedback.performHapticFeedback(HapticFeedbackType.SegmentTick)
                            newOrder = NoteOrder.Title(newOrder.orderType)
                        },
