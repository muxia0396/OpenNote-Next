...
fun TextEditorBase(
    ...
    LaunchedEffect(findAndReplaceState.searchWord) {
        currentIndex = -1
    }
    ...
    LaunchedEffect(
        ...
        if (indices.isNotEmpty() && textLayoutResult != null && findAndReplaceState.scrollDirection != null) {
            val matchTops = indices.map { textLayoutResult.getBoundingBox(it.first).top }
            val viewportHeight = (textLayoutResult.size.height - scrollState.maxValue)
                .coerceAtLeast(1)
                .toFloat()
            val viewportCenter = scrollState.value + viewportHeight / 2f
            val selectedIsStillCentered = currentIndex in matchTops.indices &&
                    kotlin.math.abs(matchTops[currentIndex] - viewportCenter) <=
                    (viewportHeight * 0.15f).coerceAtLeast(48f)

            currentIndex = if (selectedIsStillCentered) {
                ...
                when (findAndReplaceState.scrollDirection) {
                    ScrollDirection.NEXT -> matchTops.indexOfFirst { it > viewportCenter }
                        .takeIf { it >= 0 } ?: 0
                    ScrollDirection.PREVIOUS -> matchTops.indexOfLast { it < viewportCenter }
                        .takeIf { it >= 0 } ?: indices.lastIndex
                }
            }
            ...
            val scrollPosition = (bounds.top - viewportHeight / 2f)
                .toInt()
